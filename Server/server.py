        ...
        if self.path == "/register":
            ...
            for user in users:
                if user.get("username") == username and user.get("password") == hashed_password:
                    ...
                    add_session(token, user, user.get("role"))
                    self.send_response(200)
                    self.send_header("Content-type", "application/json")
                    self.end_headers()
    ...
    def do_GET(self):
        if self.path == "/profile":
            ...
            if not session_user:
                ...
                self.send_header("Content-type", "application/json")
                self.end_headers()
                self.wfile.write(b"Unauthorized: No user found")
                return

            if lid:
                print("LID FOUND")  # Check if there is a lid
                if lid not in parking_lots:
                    ...
                    return
                if 'sessions' in self.path:
                    # Check if there is sessions in a path
                    print("SESSIONS IN PATH")
                    if not token or not get_session(token):
                        self.send_response(401)
                        self.send_header("Content-type", "application/json")
                        self.end_headers()
                    ...
                    if len(parts) >= 3 and parts[2] == "sessions":
                        if len(parts) == 4:
                            sid = parts[3]
                            print("PATH CHECK 1")  # Check the path
                        ...
                        if not "ADMIN" == session_user.get('role') and not session_user.get('user') == sessions[sid].get("user"):
                            # User not an admin
                            print(
                                f"USER IS NOT AN ADMIN: {session_user.get('role')}")
                            ...
                            if "ADMIN" == session_user.get('role'):
                                ...
                                # User is an admin
                                print(
                                    f"USER IS AN ADMIN: {session_user.get('role')}")
                            else:
                                for sid, session in sessions.items():
                                    if session.get('user') == session_user.get('user'):
